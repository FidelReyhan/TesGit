...
import com.AlfaFidelReyhan.repository.CustomerRepository;
...
public class CustomerController {
    ...
    public String addCustomer(@Valid Customer customer, BindingResult result, Model model, Pageable pageable) {
        ...
        model.addAttribute("product", customerRepository.findAll(pageable));
	 ...
	 @GetMapping("/edit/{id}") public String showUpdateForm(@PathVariable("id")
	  long id, Model model) { Customer customer =
	  customerRepository.findById(id) .orElseThrow(() -> new
	  IllegalArgumentException("Invalid  id:" + id));
	  
	  model.addAttribute("customer", customer); return "operations/update-customer";
	  }
	  
	  @PostMapping("/update/{id}") public String updateUser(@PathVariable("id")
	  long id, @Valid Customer customer, BindingResult result, Model model,
	  Pageable pageable) { if (result.hasErrors()) { customer.setId(id);
	  return "operations/update-customer"; }
	  
	  customerRepository.save(customer); model.addAttribute("customer",
	  customerRepository.findAll(pageable)); return "redirect:/list"; }
